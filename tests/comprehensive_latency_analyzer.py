#!/usr/bin/env python3
"""
Enhanced Comprehensive LLM Latency Analyzer
Multi-layered data collection with detailed timing breakdowns

This tool provides:
1. Input length analysis (1-2000+ chars)
2. Output length analysis (1-200 tokens)
3. Time to First Token (TTFT) analysis
4. KV Cache and prefill timing
5. Per-token generation timing
6. Performance curve analysis
7. Cold vs warm start analysis
8. Statistical analysis with confidence intervals
"""
...
class EnhancedTestResult:
    """Enhanced test result with detailed timing breakdowns"""
    # Test metadata
    test_id: str
    test_name: str
    timestamp: str
    ...
    # Enhanced timing metrics (with defaults)
    time_to_first_token_ms: Optional[float] = None
    prefill_time_ms: Optional[float] = None
    generation_time_per_token_ms: Optional[float] = None
    error: str = ""
...
class EnhancedLatencyAnalyzer:
    ...
    def analyze_prompt_complexity(self, prompt: str) -> str:
        """Analyze prompt complexity based on length"""
        if len(prompt) < 50:
            return "simple"
        elif len(prompt) < 200:
            return "medium"
    ...
    def run_enhanced_test(self, prompt: str, max_tokens: int, 
                         iteration: int = 1, cold_start: bool = False) -> EnhancedTestResult:
        """Run an enhanced test with detailed timing metrics"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Extract enhanced timing if available
                ...
                # Calculate actual prefill time and generation time per token (only if we have real TTFT data)
                if time_to_first_token_ms is not None:
                    ...
                    # No estimates - only use actual measured data
    ...
    def run_input_length_analysis(self):
        """Test different input lengths systematically"""
        ...
        for prompt, length in prompts:
            for max_tokens in [20, 50, 100]:
                result = self.run_enhanced_test(prompt, max_tokens)
                self.results.append(result)
                time.sleep(2)
    
    def run_output_length_analysis(self):
        """Test different output lengths systematically"""
        ...
        for max_tokens in [1, 5, 10, 20, 30, 50, 75, 100, 125, 150, 175, 200]:
            result = self.run_enhanced_test(base_prompt, max_tokens)
            self.results.append(result)
            time.sleep(2)
    ...
    def run_cold_warm_analysis(self):
        ...
        cold_result = self.run_enhanced_test(test_prompt, 20, cold_start=True)
        self.results.append(cold_result)
        self.cold_start_detected = True
        
        print("Testing warm starts...")
        for i in range(10):
            warm_result = self.run_enhanced_test(test_prompt, 20, iteration=i+1, cold_start=False)
            self.results.append(warm_result)
            time.sleep(1)
    
    def run_complexity_analysis(self):
        """Test different types of prompt complexity"""
        print("\nCOMPLEXITY ANALYSIS")
        print("=" * 50)
        ...
        for prompt in simple_prompts + medium_prompts + complex_prompts:
            result = self.run_enhanced_test(prompt, 50)
            self.results.append(result)
            time.sleep(2)
    ...
    def run_batch_analysis(self):
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = []
            for i in range(5):
                future = executor.submit(self.run_enhanced_test, base_prompt, 30)
    ...
    def generate_enhanced_report(self):
        ...
        print("\nENHANCED LATENCY ANALYSIS REPORT")
        print("=" * 60)
        ...
        # Enhanced timing analysis (only if we have actual measured data)
        ...
        # Cold vs warm start analysis (same prompt comparison)
        cold_results = [r for r in successful_results if r.cold_start]
        
        if cold_results:
            print(f"\nCOLD vs WARM START:")
            # Find warm start tests with same prompt characteristics as cold start
            cold_test = cold_results[0]  # Should be only one
            ...
            if warm_same_prompt:
                ...
                # Cold start penalty should be positive when cold is slower than warm
    ...
    def save_results_to_csv(self, filename: str = "enhanced_latency_results.csv"):
    ...
    def create_enhanced_visualizations(self):
        """Create enhanced, readable visualizations"""
        try:
            successful_results = [r for r in self.results if r.success]
            ...
            fig.suptitle('Enhanced LLM Latency Analysis - Performance Curves & Bottlenecks', fontsize=16)
            
            # 1. Performance Curve: Tokens per Second vs Output Length
            ax1 = axes[0, 0]
            ...
            # 2. Latency vs Output Length (Clear Performance Zones)
            ax2 = axes[0, 1]
            ...
            # 3. Time to First Token Analysis
            ax3 = axes[0, 2]
            ...
            # 5. Cold vs Warm Start Performance
            ax5 = axes[1, 1]
            ...
            # 6. Performance Heatmap: Input vs Output Length
            ax6 = axes[1, 2]
            ...
            chart_path = os.path.join(data_dir, 'enhanced_latency_analysis.png')
            plt.savefig(chart_path, dpi=300, bbox_inches='tight')
            print(f"Enhanced charts saved to '{chart_path}'")
    ...
    def _get_input_range(self, length):
        """Helper to categorize input length"""
        if length <= 50:
            return '0-50'
        elif length <= 200:
            return '51-200'
    ...
    def _get_output_range(self, tokens):
        """Helper to categorize output length"""
        if tokens <= 25:
            return '1-25'
        elif tokens <= 50:
            return '26-50'
...
def main():
    ...
    print("Enhanced Comprehensive LLM Latency Analyzer for Blog Post Research")
    print("=" * 70)
    
    analyzer = EnhancedLatencyAnalyzer()
    ...
    print("Check the CSV file and enhanced charts for detailed data")
